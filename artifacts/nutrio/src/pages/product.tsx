...
function useGetProductByBarcode(barcode: string) {
              ...
              <div className={`px-4 py-3 flex items-center gap-2 ${grade === "E" ? "bg-red-500" : "bg-orange-500"}`}>
                ...
                <div>
                  ...
                  <p className="text-white/80 text-xs">
                    {grade === "E" ? "Yeh toh bilkul nahi yaar — try these instead:" : "Grade D hai — in options pe switch karo:"}
                  </p>
                </div>
              </div>
                  ...
                  const content = (
                    <div className="px-4 py-3 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {alt.reason && <p className="text-xs text-green-700 mt-0.5 leading-snug">{alt.reason}</p>}
                      </div>
                      {!isCurated && <ArrowRight size={15} className="text-muted-foreground flex-shrink-0" />}
                    </div>
                  );
                  return (
                    <motion.div key={`${alt.barcode}-${i}`} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.08 }}>
                      {isCurated ? <div>{content}</div> : (
            ...
            <div className="space-y-2">
              {product.tips.map((tip, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                  className={`p-3 rounded-xl text-sm leading-relaxed border ${isUnhealthy ? "bg-red-50 border-red-100 text-red-900" : "bg-amber-50 border-amber-100 text-amber-900"}`}>
                  {tip}
                </motion.div>
              ))}
            </div>
